...
func TestDecrypt(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		decrypt, err := Decrypt([]byte(content_16), key_16)
		t.Assert(err, nil)
		t.Assert(decrypt, content)

		decrypt, err = Decrypt([]byte(content_24), key_24)
		t.Assert(err, nil)
		t.Assert(decrypt, content)

		decrypt, err = Decrypt([]byte(content_32), key_32)
		t.Assert(err, nil)
		t.Assert(decrypt, content)

		decrypt, err = Decrypt([]byte(content_16_iv), key_16, iv)
		t.Assert(err, nil)
		t.Assert(decrypt, content)

		decrypt, err = Decrypt([]byte(content_32_iv), key_32, iv)
		t.Assert(err, nil)
		t.Assert(decrypt, content)

		decrypt, err = Decrypt([]byte(content_32_iv), keys, iv)
...
func TestDecryptErr(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		// decrypt key error
		encrypt, err := Encrypt(content, key_16)
		_, err = Decrypt(encrypt, key_err)
		t.AssertNE(err, nil)

		// decrypt content too short error
		_, err = Decrypt([]byte("test"), key_16)
		t.AssertNE(err, nil)

		// decrypt content size error
		_, err = Decrypt(key_17, key_16)
		t.AssertNE(err, nil)
	})
}
...
func TestDecryptCFB(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		decrypt, err := DecryptCFB([]byte(content_16_cfb), key_16, padding_size, iv)
		t.Assert(err, nil)
		t.Assert(decrypt, content)
	})
}
