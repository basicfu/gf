...
func Test_ZipPath(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(ZipPath(srcPath, dstPath), nil)
		...
		t.Assert(UnZipFile(dstPath, tempDirPath), nil)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(gfile.Exists(dstPath), false)
		err := ZipPath(srcPath1+","+srcPath2, dstPath)
		t.Assert(err, nil)
		t.Assert(gfile.Exists(dstPath), true)
		defer gfile.Remove(dstPath)
		...
		tempDirPath := gfile.TempDir(gtime.TimestampNanoStr())
		t.Assert(gfile.Mkdir(tempDirPath), nil)
		err = UnZipFile(dstPath, tempDirPath)
		t.Assert(err, nil)
		defer gfile.Remove(tempDirPath)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err = UnZipContent(zipContent, tempDirPath)
		t.Assert(err, nil)
		defer gfile.Remove(tempDirPath)
...
func Test_ZipPathWriter(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err = ZipPathWriter(srcPath1+", "+srcPath2, writer)
		t.Assert(err, nil)
