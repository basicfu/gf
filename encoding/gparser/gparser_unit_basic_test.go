...
func Test_Convert(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		p := New(`{"name":"gf","bool":true,"int":1,"float":1,"ints":[1,2],"floats":[1,2],"time":"2019-06-12","person": {"name": "gf"}}`)
...
func Test_Convert2(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(err, nil)
		t.Assert(arr, xmlArr)
		arr, err = VarToXml(`<root></root>`, "root")
		t.Assert(err, nil)
		t.Assert(arr, xmlArr)
		...
		t.Assert(err, nil)
		t.Assert(arr, xmlArr)
		arr, err = VarToXmlIndent(`<root></root>`, "root")
		t.Assert(err, nil)
		t.Assert(arr, xmlArr)
		...
		str, err = VarToJsonString(`{"name":"gf"}`)
		t.Assert(err, nil)
		t.Assert(str, `{"name":"gf"}`)
		...
		arr, err = VarToJsonIndent(`{"name":"gf"}`)
		t.Assert(err, nil)
		t.Assert(arr, jsonIndentArr)
		...
		t.Assert(err, nil)
		t.Assert(str, "{\n\t\"name\": \"gf\"\n}")
		str, err = VarToJsonIndentString(`{"name":"gf"}`)
		t.Assert(err, nil)
		t.Assert(str, "{\n\t\"name\": \"gf\"\n}")
		...
		arr, err = VarToYaml(g.Map{"name": "gf"})
		t.Assert(err, nil)
		t.Assert(arr, "name: gf\n")
