...
func TestUTF8To(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := UTF8To(data.otherEncoding, data.utf8)
		if err != nil {
			t.Errorf("Could not create decoder for %v", err)
			continue
		}
...
func TestToUTF8(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := ToUTF8(data.otherEncoding, data.other)
		if err != nil {
			t.Errorf("Could not create decoder for %v", err)
			continue
		}
...
func TestConvertErr(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		s1, e1 := Convert(srcCharset, srcCharset, src)
		t.Assert(e1, nil)
		t.Assert(s1, src)

		s2, e2 := Convert(dstCharset, "no this charset", src)
		t.AssertNE(e2, nil)
		t.Assert(s2, src)

		s3, e3 := Convert("no this charset", srcCharset, src)
		t.AssertNE(e3, nil)
		t.Assert(s3, src)
	})
}
...
func TestSupported(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		t.Assert(Supported("UTF-8"), true)
		t.Assert(Supported("UTF-80"), false)
	})
}
