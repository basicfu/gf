...
func ExampleArray_Iterator() {
	array := NewArrayFrom(g.Slice{"a", "b", "c"})
...
func ExampleArray_Reverse() {
	array := NewFrom(g.Slice{1, 2, 3, 4, 5, 6, 7, 8, 9})
...
func ExampleArray_Merge() {
	array1 := NewFrom(g.Slice{1, 2})
	array2 := NewFrom(g.Slice{3, 4})
	slice1 := g.Slice{5, 6}
	slice2 := []int{7, 8}
	slice3 := []string{"9", "0"}
...
func ExampleArray_FilterEmpty() {
	array1 := NewFrom(g.Slice{0, 1, 2, nil, "", g.Slice{}, "john"})
	array2 := NewFrom(g.Slice{0, 1, 2, nil, "", g.Slice{}, "john"})
