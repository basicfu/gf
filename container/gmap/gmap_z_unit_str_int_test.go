...
func Test_StrIntMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := NewStrIntMapFrom(map[string]int{"a": 1, "b": 2})
...
func Test_StrIntMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		//clone 方法是深克隆
		m := NewStrIntMapFrom(map[string]int{"a": 1, "b": 2, "c": 3})
...
func Test_StrIntMap_Pop(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m := NewStrIntMapFrom(g.MapStrInt{
			"k1": 11,
			"k2": 22,
		})
		t.Assert(m.Size(), 2)
