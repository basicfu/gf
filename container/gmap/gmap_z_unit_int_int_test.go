...
func Test_IntIntMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := NewIntIntMapFrom(map[int]int{1: 1, 2: 2})
...
func Test_IntIntMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		//clone 方法是深克隆
		m := NewIntIntMapFrom(map[int]int{1: 1, 2: 2})
...
func Test_IntIntMap_Pop(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m := NewIntIntMapFrom(g.MapIntInt{
			1: 11,
			2: 22,
		})
		t.Assert(m.Size(), 2)
