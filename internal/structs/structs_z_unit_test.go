...
func Test_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		var user User
		m, _ := TagMapName(user, []string{"params"})
		t.Assert(m, g.Map{"name": "Name", "pass": "Pass"})
		m, _ = TagMapName(&user, []string{"params"})
		t.Assert(m, g.Map{"name": "Name", "pass": "Pass"})

		m, _ = TagMapName(&user, []string{"params", "my-tag1"})
		t.Assert(m, g.Map{"name": "Name", "pass": "Pass"})
		m, _ = TagMapName(&user, []string{"my-tag1", "params"})
		t.Assert(m, g.Map{"name": "Name", "pass1": "Pass"})
		m, _ = TagMapName(&user, []string{"my-tag2", "params"})
	...
	gtest.C(t, func(t *gtest.T) {
		...
		m, _ := TagMapName(user1, []string{"params"})
		t.Assert(m, g.Map{"password1": "Pass1", "password2": "Pass2"})
		m, _ = TagMapName(user2, []string{"params"})
		t.Assert(m, g.Map{})
	})
}
