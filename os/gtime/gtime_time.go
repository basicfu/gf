	...
	if len(param) > 0 {
		...
		case string:
			if len(param) > 1 {
				...
				case string:
					return StrToTimeFormat(r, t)
				case []byte:
					return StrToTimeFormat(r, string(t))
				}
			}
			return StrToTime(r)
		case []byte:
			if len(param) > 1 {
				switch t := param[1].(type) {
				case string:
					return StrToTimeFormat(string(r), t)
				case []byte:
					return StrToTimeFormat(string(r), string(t))
				}
			}
