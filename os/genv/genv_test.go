...
func Test_GEnv_Contains(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err := os.Setenv(key, "TEST")
		t.Assert(err, nil)
		t.AssertEQ(Contains(key), true)
		t.AssertEQ(Contains("none"), false)
	})
}
...
func Test_GetWithCmd(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		gcmd.Init("-test", "2")
		t.Assert(GetWithCmd("TEST"), 2)
	})
	gtest.C(t, func(t *gtest.T) {
		Set("TEST", "1")
		defer Remove("TEST")
		gcmd.Init("-test", "2")
		t.Assert(GetWithCmd("test"), 1)
	})
}
