...
func TestWatcher_AddOnce(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		callback1, err := AddOnce("mywatch", path, func(event *Event) {
			value.Set(1)
		})
		t.Assert(err, nil)
		callback2, err := AddOnce("mywatch", path, func(event *Event) {
			value.Set(2)
		})
		t.Assert(err, nil)
		...
		err = RemoveCallback(callback1.Id)
		t.Assert(err, nil)
...
func TestWatcher_AddRemove(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		v := gtype.NewInt(1)
		callback, err := Add(path1, func(event *Event) {
			if event.IsWrite() {
				v.Set(2)
				return
			}
...
func TestWatcher_Callback2(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		v1 := gtype.NewInt(1)
		v2 := gtype.NewInt(1)
		callback1, err1 := Add(path1, func(event *Event) {
			if event.IsWrite() {
				v1.Set(2)
				return
			}
		})
		callback2, err2 := Add(path1, func(event *Event) {
			if event.IsWrite() {
				v2.Set(2)
				return
			}
