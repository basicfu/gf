...
func TestCfg_Config(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		SetContent("gf", "config.yml")
		t.Assert(GetContent("config.yml"), "gf")
		SetContent("gf1", "config.yml")
		t.Assert(GetContent("config.yml"), "gf1")
		RemoveContent("config.yml")
		ClearContent()
		t.Assert(GetContent("name"), "")
	})
}
