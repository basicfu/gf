...
func Test_CaseDelimitedScreaming(t *testing.T) {
	...
	for _, i := range cases {
		in := i[0]
		out := i[1]
		result := CaseDelimitedScreaming(in, '.', true)
		if result != out {
			t.Error("'" + result + "' != '" + out + "'")
		}
	}
...
func Test_CaseSnakeFirstUpper(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		for _, item := range cases {
			t.Assert(CaseSnakeFirstUpper(item[0]), item[1])
		}
	})
