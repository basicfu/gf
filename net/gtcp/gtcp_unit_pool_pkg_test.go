...
func Test_Pool_Package_Basic(t *testing.T) {
	p, _ := ports.PopRand()
	s := NewServer(fmt.Sprintf(`:%d`, p), func(conn *Conn) {
		defer conn.Close()
		for {
	...
	gtest.C(t, func(t *gtest.T) {
		conn, err := NewPoolConn(fmt.Sprintf("127.0.0.1:%d", p))
		t.Assert(err, nil)
		defer conn.Close()
		for i := 0; i < 100; i++ {
...
func Test_Pool_Package_Option(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		result, err := conn.SendRecvPkg(data, PkgOption{HeaderSize: 1})
		t.AssertNE(err, nil)
		t.Assert(result, nil)
	})
	// SendRecvPkg with big data - success.
