...
func Test_Config2(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(Config().Get("test"), "v=1")
		t.Assert(Config().Get("database.default.1.host"), "127.0.0.1")
		t.Assert(Config().Get("redis.disk"), "127.0.0.1:6379,0")
	})
...
func Test_Config3(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err = Config("test").AddPath(dirPath)
		t.Assert(err, nil)

		defer Config("test").Clear()
		Config("test").SetFileName("test.toml")

		t.Assert(Config("test").Get("test"), "v=1")
		t.Assert(Config("test").Get("database.default.1.host"), "127.0.0.1")
		t.Assert(Config("test").Get("redis.disk"), "127.0.0.1:6379,0")
	})
	// for gfsnotify callbacks to refresh cache of config file
	time.Sleep(500 * time.Millisecond)
...
func Test_Config4(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(Config().AddPath(path), nil)
		t.Assert(Config().Get("test"), "v=1")
		t.Assert(Config().Get("database.default.1.host"), "127.0.0.1")
		t.Assert(Config().Get("redis.disk"), "127.0.0.1:6379,0")
	})
	...
	gtest.C(t, func(t *gtest.T) {
		...
		defer Config("test").Clear()
		Config("test").SetFileName("test.toml")
		t.Assert(Config("test").AddPath(path), nil)
		t.Assert(Config("test").Get("test"), "v=1")
		t.Assert(Config("test").Get("database.default.1.host"), "127.0.0.1")
		t.Assert(Config("test").Get("redis.disk"), "127.0.0.1:6379,0")
	})
...
func Test_Basic2(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(Config().Get("log-path"), "logs")
	})
}
