...
func Test_Struct_Attr_CustomType2(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		user := new(User)
		err := Struct(g.Map{"id": g.Slice{1, 2}, "name": "john"}, user)
		t.Assert(err, nil)
...
// https://github.com/basicfu/gf/issues/775
...
func Test_Struct_To_Struct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		t.Assert(Struct(TestB, &TestA), nil)
...
func Test_Struct_JsonParam(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		var a = A{}
		err := Struct([]byte(`{"id":1,"name":"john"}`), &a)
		t.Assert(err, nil)
		t.Assert(a.Id, 1)
