...
func (st *MyTimeSt) UnmarshalValue(v interface{}) error {
	m := Map(v)
	t, err := gtime.StrToTime(String(m["ServiceDate"]))
	if err != nil {
		return err
	}
...
func Test_Struct_UnmarshalValue1(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		st := &MyTimeSt{}
		err := Struct(g.Map{"ServiceDate": "2020-10-10 12:00:01"}, st)
		t.Assert(err, nil)
		t.Assert(st.ServiceDate.Time.Format("2006-01-02 15:04:05"), "2020-10-10 12:00:01")
	})
	gtest.C(t, func(t *gtest.T) {
		st := &MyTimeSt{}
		err := Struct(g.Map{"ServiceDate": nil}, st)
		t.AssertNE(err, nil)
	})
	gtest.C(t, func(t *gtest.T) {
		st := &MyTimeSt{}
		err := Struct(g.Map{"ServiceDate": "error"}, st)
		t.AssertNE(err, nil)
	})
}
