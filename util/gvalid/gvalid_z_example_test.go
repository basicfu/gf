...
func ExampleCheckMap() {
	...
	if e := CheckMap(params, rules); e != nil {
		fmt.Println(e.Map())
...
func ExampleRegisterRule() {
	rule := "unique-name"
	RegisterRule(rule, func(rule string, value interface{}, message string, params map[string]interface{}) error {
		var (
...
func ExampleRegisterRule_OverwriteRequired() {
	...
	fmt.Println(Check("", "required", "It's required"))
	fmt.Println(Check([]string{}, "required", "It's required"))
	fmt.Println(Check(map[string]int{}, "required", "It's required"))
	DeleteRule(rule)
